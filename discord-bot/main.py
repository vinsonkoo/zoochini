    ...
    try:
        async for message in channel.history(limit=limit):
            ...
            if message.attachments:
                tasks = [get_file_content(attachment)
...
async def ask(interaction: discord.Interaction, question: str):
    try:
        ...
        # Split on sentence boundaries instead of fixed length
        chunks = [chunk.strip() for chunk in response.split('. ')]
        current_chunk = []
        current_length = 0

        for sentence in chunks:
            if current_length + len(sentence) > 1900:  # Leave room for punctuation
            await interaction.followup.send('. '.join(current_chunk) + '.')
